...
export default function OfferCard({ offer, isFavorite, onToggleFavorite, onPress, onApply }: OfferCardProps) {
  ...
  // Определяем видимые поля (как в PHP)
    ...
    >
            ...
            {offer.rating > 0 && (
              <View style={styles.ratingBadge}>
                ...
                {offer.review_count > 0 && (
                  <Text style={styles.reviewCount}> ({offer.review_count})</Text>
                )}
              </View>
            )}
            {showFreeTerm && (
        ...
        {showAmount && (
          <View style={styles.metric}>
            ...
            <Text style={styles.metricValue} numberOfLines={1}>
              {formatMoney(offer.amount_min)} — {formatMoney(offer.amount_max)}
            </Text>
          </View>
        )}
        {showTerm && (
          <View style={styles.metric}>
            ...
            <Text style={styles.metricValue} numberOfLines={1}>
              {formatDays(offer.term_min_days)} — {formatDays(offer.term_max_days)}
            </Text>
          </View>
        )}
        {showRate && (
          <View style={styles.metric}>
            ...
            <Text style={styles.metricValue} numberOfLines={1}>
              {formatRate(offer.rate, rateUnit)}
            </Text>
          </View>
        )}
        {showPsk && (
      ...
      <View style={styles.actions}>
        <View style={styles.actionsLeft}>
          ...
          <TouchableOpacity
            onPress={() => onToggleFavorite?.(offer.id)}
            style={styles.favBtn}
          >
