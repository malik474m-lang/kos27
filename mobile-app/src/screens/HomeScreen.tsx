...
import type { Offer, GiveawayInfo } from '../api/types';
  ...
  return (
    ...
    >
      ...
      <LinearGradient colors={[Colors.gradientStart, Colors.gradientEnd]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
        <Text style={styles.heroTitle}>Подберите лучший{'\n'}займ или кредит</Text>
        <Text style={styles.heroSubtitle}>Сравнивайте условия от проверенных партнёров</Text>
      ...
      <View style={styles.categoriesRow}>
        <CategoryCard icon="💵" title="Займы" subtitle="Микрозаймы онлайн" onPress={() => navigation.navigate('Catalog', { category: 'microloans' })} />
        <CategoryCard icon="🏦" title="Кредиты" subtitle="Банковские" onPress={() => navigation.navigate('Catalog', { category: 'credits' })} />
      </View>
      <View style={styles.categoriesRow}>
        <CategoryCard icon="💳" title="Кредитные" subtitle="С лимитом" onPress={() => navigation.navigate('Catalog', { category: 'credit_cards' })} />
        <CategoryCard icon="🪪" title="Дебетовые" subtitle="С кэшбеком" onPress={() => navigation.navigate('Catalog', { category: 'debit_cards' })} />
      </View>
      ...
      {giveaway && (
        <TouchableOpacity style={styles.giveawayBanner} onPress={() => Linking.openURL('https://kosmozaim.ru/giveaway')}>
          ...
          <View style={styles.giveawayContent}>
            ...
            <Text style={styles.giveawayPrize}>Приз: {formatMoney(giveaway.prize_amount)}</Text>
          </View>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('Calculator')}>
        <LinearGradient colors={[Colors.primary, '#7e3af2']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.calcBanner}>
      ...
      <View style={styles.navSection}>
        ...
        <View style={styles.navGrid}>
          {[
            ...
            <TouchableOpacity key={item.title} style={styles.navItem} onPress={item.onPress}>
...
const styles = StyleSheet.create({
  ...
  giveawayContent: { flex: 1 },
  giveawayTitle: { fontSize: 15, fontWeight: '700', color: '#92400e' },
  giveawayPrize: { fontSize: 13, color: '#b45309', marginTop: 2 },
  giveawayArrow: { fontSize: 18, color: '#b45309', fontWeight: '700' },
  section: { paddingHorizontal: 16, marginTop: 24 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  sectionTitle: { fontSize: 20, fontWeight: '700', color: Colors.text },
  sectionLink: { fontSize: 14, color: Colors.primary, fontWeight: '500' },
  calcBanner: { marginHorizontal: 16, marginTop: 24, padding: 24, borderRadius: 20, alignItems: 'center' },
  calcBannerTitle: { fontSize: 20, fontWeight: '700', color: Colors.white, marginBottom: 8 },
  calcBannerSub: { fontSize: 14, color: 'rgba(255,255,255,0.8)', textAlign: 'center' },
  navSection: { paddingHorizontal: 16, marginTop: 24 },
  navGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: 16 },
  navItem: { width: '47%' as any, backgroundColor: Colors.white, borderRadius: 12, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: Colors.surfaceBorder },
