...
</tr></thead><tbody id="chk-tb"></tbody></table></div></div>
...
</tr></thead><tbody id="aud-tb"></tbody></table></div></div>
</main>
<div id="modal"></div>
<script>
const A='/api/admin';let plans=[],ch1,ch2;
function tab(n){document.querySelectorAll('.tc').forEach(e=>e.classList.add('hidden'));document.getElementById('t-'+n).classList.remove('hidden');
document.querySelectorAll('.tb').forEach(b=>{b.classList.toggle('border-purple-600',b.dataset.t===n);b.classList.toggle('text-purple-600',b.dataset.t===n);b.classList.toggle('border-transparent',b.dataset.t!==n);b.classList.toggle('text-gray-500',b.dataset.t!==n);});
if(n==='dash')loadStats();if(n==='lic')loadLic();if(n==='chk')loadChk();if(n==='aud')loadAud();}
async function loadStats(){
const[sr,pr]=await Promise.all([fetch(A+'/stats').then(r=>r.json()),fetch(A+'/plans').then(r=>r.json())]);
plans=pr.plans||[];const s=sr.stats||{};
...
const c24=s.checks_24h||{};document.getElementById('s-checks').textContent=Object.values(c24).reduce((a,b)=>a+parseInt(b),0);
...
async function createLic(e){e.preventDefault();const f=e.target;
...
else alert(r.error||'Ошибка');return false;}
async function editLic(id){const r=await fetch(A+'/licenses').then(r=>r.json());const l=r.licenses.find(x=>x.id==id);if(!l)return;
...
async function updLic(e,id){e.preventDefault();const f=e.target;
...
if(r.success){closeModal();loadLic();}else alert(r.error||'Ошибка');return false;}
async function delLic(id){if(!confirm('Удалить?'))return;const r=await fetch(A+'/licenses/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})}).then(r=>r.json());if(r.success)loadLic();else alert(r.error||'Ошибка');}
async function loadChk(){const r=await fetch(A+'/checks').then(r=>r.json());
document.getElementById('chk-tb').innerHTML=(r.checks||[]).map(c=>`<tr class="border-b hover:bg-gray-50">
...
</tr>`).join('')||'<tr><td colspan="5" class="px-4 py-8 text-center text-gray-500">Нет данных</td></tr>';}
async function loadAud(){const r=await fetch(A+'/audit').then(r=>r.json());
document.getElementById('aud-tb').innerHTML=(r.logs||[]).map(l=>`<tr class="border-b hover:bg-gray-50">
...
</tr>`).join('')||'<tr><td colspan="4" class="px-4 py-8 text-center text-gray-500">Нет данных</td></tr>';}
async function show2FA(){const me=await fetch(A+'/me').then(r=>r.json());
if(me.admin?.totp_enabled){modal(`<h2 class="text-xl font-bold mb-4">🔒 2FA включена</h2>
...
else{const s=await fetch(A+'/2fa/setup').then(r=>r.json());
...
async function en2FA(e){e.preventDefault();const r=await fetch(A+'/2fa/enable',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:e.target.code.value})}).then(r=>r.json());
...
async function dis2FA(e){e.preventDefault();const r=await fetch(A+'/2fa/disable',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:e.target.pw.value})}).then(r=>r.json());
if(r.success){closeModal();alert('2FA отключена');}else alert(r.error||'Ошибка');return false;}
function showPw(){modal(`<h2 class="text-xl font-bold mb-4">🔑 Смена пароля</h2>
...
async function chgPw(e){e.preventDefault();const r=await fetch(A+'/change-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({current_password:e.target.cur.value,new_password:e.target.new.value})}).then(r=>r.json());
if(r.success){closeModal();alert('Пароль изменён');}else alert(r.error||'Ошибка');return false;}
async function logout(){await fetch(A+'/logout',{method:'POST'});location.href='/admin/login';}
