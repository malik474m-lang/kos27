...
warnings.forEach(c=>{h+='<div class="text-sm text-yellow-700"><div class="flex items-center justify-between gap-3"><div>• '+e(c.msg)+'</div>'+(c.fixTab?'<button type="button" data-fix-tab="'+e(c.fixTab)+'" data-fix-item-type="'+e(c.fixItemType||'')+'" data-fix-item-id="'+((c.items&&c.items.length&&typeof c.items[0]==='object')?e(String(c.items[0].id||'')):'')+'" class="health-fix-btn text-xs px-2 py-1 rounded bg-yellow-100 text-yellow-800 border border-yellow-200 hover:bg-yellow-200">Исправить</button>':'')+'</div>'; if(c.items&&c.items.length){ h+='<ul class="mt-2 ml-5 list-disc text-xs text-yellow-600 space-y-1">'+c.items.slice(0,10).map(i=>{ if(typeof i==='object'){ return '<li><button type="button" class="underline hover:no-underline text-left" onclick="goHealthFix(\''+c.fixTab+'\',\''+(c.fixItemType||'')+'\',\''+i.id+'\')">'+e(i.title||'—')+'</button></li>'; } return '<li>'+e(i)+'</li>'; }).join('')+'</ul>'; } h+='</div>';});
h+='</div></div>';}
