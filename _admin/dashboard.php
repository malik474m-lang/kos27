...
function genOfferInfo(){var t=document.getElementById('of-t').value,c=document.getElementById('of-c').value;if(!t){alert('Введите название оффера');return;}var b=event.target;b.disabled=true;b.textContent='⏳';ap('/offer-contacts-generate',{method:'POST',body:JSON.stringify({title:t,category:c})}).then(function(r){if(r.error){alert('Ошибка: '+r.error+(r.raw?'

Ответ ИИ: '+r.raw:''));return;}if(r.contacts){if(r.contacts.phone!==undefined&&document.getElementById('of-phone'))document.getElementById('of-phone').value=r.contacts.phone||'';if(r.contacts.license!==undefined&&document.getElementById('of-license'))document.getElementById('of-license').value=r.contacts.license||'';if(r.contacts.trademark!==undefined&&document.getElementById('of-trademark'))document.getElementById('of-trademark').value=r.contacts.trademark||'';if(r.contacts.address!==undefined&&document.getElementById('of-address'))document.getElementById('of-address').value=r.contacts.address||'';}}).catch(function(e){alert('Ошибка: '+(e.message||e));}).finally(function(){b.disabled=false;b.textContent='🤖 ИИ';});}
