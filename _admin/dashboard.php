...
function modal(h,closeOnBackdrop=true){var bgClick=closeOnBackdrop?'if(event.target===this)cm()':'';document.getElementById('M').innerHTML='<div class="modal-bg"'+(bgClick?' onclick="'+bgClick+'"':'')+'><div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl">'+h+'</div></div>'; }
...
orderedKeys.forEach(function(key){
  ...
  items.forEach(function(o){
    ...
    h+='<button onclick="event.stopPropagation();faqGen('+o.id+',&#39;'+e(o.title).replace(/'/g,'')+'&#39;,&#39;'+e(o.slug)+'&#39;)" class="text-purple-600 hover:underline text-sm">FAQ</button>';
...
'<div class="col-span-2"><label class="block text-xs font-medium mb-1">URL логотипа</label><input id="of-lo" class="input-f w-full" value="'+e(f.logo_url||'')+'"><div class="mt-2 flex flex-wrap gap-2"><button type="button" onclick="mediaPicker(\'of-lo\',\'offer\')" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg text-xs font-semibold">📁 Выбрать из медиа</button><button type="button" id="of-logo-import-btn" onclick="importOfferLogoUrl()" class="px-3 py-2 rounded-lg text-xs font-semibold border border-orange-700" style="background:#ea580c;color:#ffffff">⬇ Скачать по URL</button><button type="button" onclick="openOfferLogoUrl()" class="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-2 rounded-lg text-xs font-semibold">↗ Открыть ссылку</button></div><p class="text-[11px] text-gray-500 mt-1">Вставьте внешнюю ссылку на логотип и нажмите «Скачать по URL» — файл сохранится локально в /images/offer.</p><div id="of-lo-preview" class="mt-2">'+(f.logo_url?'<img src="'+e(f.logo_url)+'" class="w-16 h-16 object-contain rounded border bg-white">':'')+'</div></div>'+
...
function aSanitizeMeta(){if(!confirm('Очистить HTML и мусор из title / excerpt / meta_title / meta_description у всех статей?'))return;fetch(A+'/articles/sanitize-meta',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}).then(function(r){return r.json();}).then(function(d){if(d.success){alert(d.message||('Очищено: '+d.updated));lA();}else{alert(d.error||'Ошибка');}}).catch(function(err){alert('Ошибка: '+(err&&err.message?err.message:'неизвестная'));});}
...
_sdState={scope:scope,offset:0,fixedT:0,fixedD:0,fails:0};_sdAllItems=[];
seoDupStep();
}
var _sdState=null;
function seoDupStep(){
...
_sdAllItems=_sdAllItems.concat(d.items||[]);
var rows='';
_sdAllItems.forEach(function(it){
var st=it.ok?'<span class="text-green-600">✓</span>':('<span class="text-red-500" title="'+e(it.reason||'')+'">✗</span>');
rows+='<div class="flex items-start gap-2 py-1 border-b border-gray-100 text-xs">'+st+'<span class="font-mono text-gray-500">'+e(it.url)+'</span><span class="text-gray-400">('+(it.type==='title'?'title':'desc')+') → '+e(it.value||'')+'</span></div>';
});
if(slot)slot.innerHTML='<div class="bg-green-50 rounded-lg p-3"><p class="text-green-700 text-sm font-semibold">✅ Готово</p><p class="text-xs text-green-600 mt-1">Исправлено title: '+s.fixedT+', description: '+s.fixedD+(s.fails?(', ошибок: '+s.fails):'')+'. Всего групп: '+d.total+'.</p>'+(rows?'<div class="max-h-64 overflow-y-auto mt-2 bg-white rounded p-2">'+rows+'</div>':'')+'<p class="text-xs text-gray-500 mt-2">Обновляю список дублей…</p></div>';
setTimeout(function(){ lSeoDuplicates(); }, 1200);
}
})
.catch(function(err){
var slot=document.getElementById('seo-dup-loading')||document.getElementById('seo-dup-result');
...
/* ============ FAQ ============ */
function faqGen(offerId, offerTitle, offerSlug){
...
faqLoadModal(offerId, offerTitle, offerSlug);
}

function faqLoadModal(offerId, offerTitle, offerSlug){
...
h+='<div class="flex flex-wrap gap-2">';
h+='<button onclick="faqDoGen('+offerId+',&#39;'+e(offerTitle).replace(/'/g,"")+'&#39;,&#39;'+(offerSlug||'')+'&#39;)" id="faq-gen-btn" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-semibold">🤖 '+(list&&list.length?'Перегенерировать':'Сгенерировать')+' FAQ</button>';
h+='<a href="/offer/'+(offerSlug||offerId)+'" target="_blank" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-semibold">👁 Предпросмотр</a>';
h+='</div>';
...
function faqDoGen(offerId, offerTitle, offerSlug){
...
function faqDel(id, offerId, offerTitle, offerSlug){
if(!confirm('Удалить вопрос?')) return;
ap('/faq/delete',{method:'DELETE',body:JSON.stringify({id:id})}).then(function(d){
if(d.success) faqLoadModal(offerId, offerTitle, offerSlug);
else alert('Ошибка');
});
}
...
function odiLoadKeys(){
    ...
    fetch(A+'/odirouter-keys').then(function(r){return r.json();}).then(function(d){
        ...
        Object.keys(groups).sort().forEach(function(acc){
            ...
            var accCooldown=gKeys[0].account_cooldown||0;
            var cooldownMin=Math.ceil(accCooldown/60);
            h+='<div class="mb-3 '+((accDisabled||accCooldown>0)?'opacity-50':'')+'"><div class="flex items-center gap-2 mb-1 flex-wrap"><button onclick="odiToggleAccount(this)" data-account="'+e(acc).replace(/"/g,'&quot;')+'" class="text-xs px-2 py-0.5 rounded '+(accDisabled?'bg-red-100 text-red-600':'bg-green-100 text-green-700')+'">'+(accDisabled?'⛔ Выкл':'✅ Вкл')+'</button><span class="text-xs font-bold text-gray-700">👤 '+e(acc)+'</span><span class="text-xs '+accColor+' font-medium">'+accUsed+'/50 запросов</span><span class="text-xs text-gray-400">('+gKeys.length+' ключей)</span>'+(accCooldown>0?'<span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded">⏳ '+cooldownMin+' мин</span>':'');
...
function odiSaveEdit(id){
var name=document.getElementById('odi-edit-name').value.trim();
var account=document.getElementById('odi-edit-account').value.trim();
var type=document.getElementById('odi-edit-type').value;
fetch(A+'/odirouter-keys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'update',id:id,name:name,account:account,type:type})}).then(function(r){return r.json();}).then(function(d){
if(d.success){cm();odiLoadKeys();}else{alert(d.error||'Ошибка');}
});
}
...
function lAIP(){
    ...
    fetch(A+'/ai-providers').then(r=>r.json()).then(function(d){
        ...
        '<div class="flex items-center justify-between mb-4 flex-wrap gap-2"><div class="flex items-center gap-3"><span class="text-3xl">🌐</span><div><h3 class="font-bold text-lg">OdiRouter — Пул API ключей</h3><p class="text-sm text-gray-500">Ротация по аккаунтам (50 запросов/день на аккаунт)</p></div></div>'+
