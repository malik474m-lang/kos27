...
function genOfferInfo(){var t=document.getElementById('of-t').value,c=document.getElementById('of-c').value;if(!t){alert('Введите название');return;}var b=event.target;b.disabled=true;b.textContent='...';ap('/offer-contacts-generate',{method:'POST',body:JSON.stringify({title:t,category:c})}).then(function(r){if(r.error){alert(r.error);return;}var d=r.contacts||{};if(d.phone)document.getElementById('of-phone').value=d.phone;if(d.license)document.getElementById('of-license').value=d.license;if(d.trademark)document.getElementById('of-trademark').value=d.trademark;if(d.address)document.getElementById('of-address').value=d.address;}).catch(function(e){alert(e.message||e);}).finally(function(){b.disabled=false;b.textContent='🤖 ИИ';});}
</script>
