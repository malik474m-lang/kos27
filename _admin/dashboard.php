...
<?php include __DIR__ . "/patch-yandex-direct.php"; ?>
<?php include __DIR__ . "/patch-social-proof.php"; ?>
<?php include __DIR__ . "/patch-offer-contacts.php"; ?>
...
function genOfferInfo(){var t=document.getElementById('of-t').value,c=document.getElementById('of-c').value;if(!t){alert('Введите название');return;}var b=event.target;b.disabled=true;b.textContent='⏳';ap('/offer-contacts-generate',{method:'POST',body:JSON.stringify({title:t,category:c})}).then(function(r){if(r.error){alert(r.error);return;}if(r.contacts){if(r.contacts.phone)document.getElementById('of-phone').value=r.contacts.phone;if(r.contacts.license)document.getElementById('of-license').value=r.contacts.license;if(r.contacts.trademark)document.getElementById('of-trademark').value=r.contacts.trademark;if(r.contacts.address)document.getElementById('of-address').value=r.contacts.address;}}).catch(function(e){alert(e.message||e);}).finally(function(){b.disabled=false;b.textContent='🤖 ИИ';});}
