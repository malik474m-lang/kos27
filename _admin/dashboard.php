...
⚠️ Текст: '+d.textError:'';let dbg=d.debug?'

🔍 Debug: active='+d.debug.activeText+' providers='+JSON.stringify(d.debug.providers):'';alert('Статья "'+d.article.title+'" создана!
🤖 '+d.aiProvider+im+errNote+textErr+dbg);}else alert('Ошибка: '+(d.error||d.textError||'неизвестная'));lA();}).catch(()=>{alert('Ошибка');b.disabled=false;b.textContent='🚀 Сгенерировать';});}
