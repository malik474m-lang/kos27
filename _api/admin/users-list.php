...
try {
    $users = $db->query("
        SELECT u.id, u.email, u.name, u.is_verified, u.last_login_at, u.last_login_ip,
               u.agreed_terms, u.agreed_marketing, u.agreed_finance, u.created_at, u.bonus_balance,
    ...
    try {
        $users = $db->query("SELECT *, COALESCE(bonus_balance,0) as bonus_balance, 0 as app_count, 0 as approved_count FROM users ORDER BY created_at DESC LIMIT 200")->fetchAll();
        apiCacheEnd($users);
    } catch (Exception $e2) {
        apiCacheEnd([]);
    }
