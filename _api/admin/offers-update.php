...
try {
    $db->prepare("UPDATE offers SET title=?, category=?, amount_min=?, amount_max=?, term_min_days=?, term_max_days=?, psk=?, rate=?, free_term_days=?, logo_url=?, affiliate_url=?, borrower_category=?, description=?, seo_keywords=?, regions=?, is_active=?, sort_order=?, extra_fields=?, display_fields=? WHERE id=?")
    ->execute([
    ...
    try {
        ...
        echo json_encode(['success' => true, 'warning' => 'Сохранено без extra/display fields. Выполните SQL-миграции.']);
    } catch (Exception $e2) {
